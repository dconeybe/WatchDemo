...
def populate(stub: firestore_pb2_grpc.FirestoreStub) -> None:
  for i in range(5):
...
def list_documents(stub: firestore_pb2_grpc.FirestoreStub) -> None:
  ...
  for document in response.documents:
    count += 1
    logging.info("#%s: %s key=%s", count, document.name, str(document.fields.get("key")).strip())
...
class UpdateDocument:
  ...
  def __call__(self, stub: firestore_pb2_grpc.FirestoreStub) -> None:
...
def main(argv: Sequence[str]) -> None:
  if len(argv) == 1:
    raise app.UsageError("no command specified; supported commands are: populate ls")
  elif len(argv) == 2:
    command_str = argv[1]
  elif len(argv) > 2:
    raise app.UsageError(f"unexpected argument: {argv[2]}")

  if command_str == "populate":
    command = populate
  elif command_str == "ls":
    ...
    command = UpdateDocument(document_id=match.group(1), value=int(match.group(2)))
  else:
